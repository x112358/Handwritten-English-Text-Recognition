...
int main(int argc, char *argv[]) {
    ...
    for (int i=0; i<lines.size(); i++) {
        ...
        string seperate = to_string((i+1)*1e-6).substr(5);
        wordsPath /= seperate;
		fs::create_directories(wordsPath);
       
        for (int j=0; j<words.size(); j++) {
        
             //string seperate = "line_"+ to_string((j+1)*1e-6).substr(5);
             //fs::path seperatePath = seperate;
             //fs::create_directories(seperatePath);          
